...
function repoArg(argv: string[]): string {
  if (argv.includes("--help") || argv.includes("-h")) throw new Error("Usage: npm run stax:gate -- --repo <path>");
...
async function main(): Promise<void> {
  ...
  if (argv.includes("--help") || argv.includes("-h")) {
    process.stdout.write("Usage: npm run stax:gate -- --repo <path>\n");
    return;
  }
  const status = await runStaxGate({ repoPath: repoArg(argv) });
