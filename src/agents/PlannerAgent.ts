  ...
  async execute(input: AgentInput): Promise<AgentResult> {
    ...
    if (input.mode === "prompt_factory") {
      return {
        ...
        output: [
          ...
          "",
          "## Acceptance Criteria",
          "- The change preserves existing explicit domain-signal behavior.",
    ...
    return {
      ...
      output: [
        ...
        "",
        "## Current State",
        "- STAX is a local proof gate for AI-coded work; stax_fitness is a legacy explicit domain-signal compatibility mode, not the product identity.",
        "- Existing behavior must stay governed by proof evidence, policies, schemas, critic review, validation, traces, evals, corrections, and approved memory.",
        "",
        "## Concrete Changes Required",
        "1. Identify the proof-gate, mode, schema, runtime, eval, and CLI surfaces touched by the requested behavior.",
        "2. Add behavior-first tests that prove the route, output contract, trace, queue, gate verdict, or promotion boundary.",
        "3. Implement the bounded proof-gate or runtime change while preserving mock provider and approval gates.",
