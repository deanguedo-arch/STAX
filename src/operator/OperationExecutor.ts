...
export type OperationExecutorHandlers = {
  ...
  issueTriage: (plan: OperationPlan) => Promise<OperationExecutionResult>;
  codexReportAudit: (plan: OperationPlan) => Promise<OperationExecutionResult>;
...
export class OperationExecutor {
  ...
  async execute(plan: OperationPlan, handlers: OperationExecutorHandlers): Promise<OperationExecutionResult> {
    ...
    if (plan.intent === "issue_triage") return handlers.issueTriage(plan);
    if (plan.intent === "codex_report_audit") return handlers.codexReportAudit(plan);
